...
function consolidateFAQs(faqData: any[]) {
  // Initialize an empty array to hold all consolidated FAQs
  const consolidatedFAQs: any[] = [];

  // Iterate over each item in the input array
  faqData.forEach((item) => {
    // Check if the item has an 'faqs' array
    if (item.faqs && Array.isArray(item.faqs)) {
      // Iterate over each FAQ in the 'faqs' array
      item.faqs.forEach((faq) => {
        // Add the FAQ to the consolidatedFAQs array
        consolidatedFAQs.push(faq);
      });
    }
  });

  // Return the consolidated FAQs array
  return consolidatedFAQs;
}
...
const CityDetailPage = async ({ params }: CityDetailPageProps) => {
  ...
  if (
    currentCityResponse.success === false ||
    attorneysResponse.success === false ||
    articlesResponse.success === false ||
    faqsResponse.success === false ||
  ...
  const faqs = faqsResponse.data && consolidateFAQs(faqsResponse.data);
  const reviews = reviewsResponse.data;
  console.log("currentCIty", currentCityResponse);
  console.log("articles", faqs);
  return (
    <section className="bg-white dark:bg-gray-900">
      <div className="container border-sm px-6 py-8 mx-auto">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1572978122938-4b7b95c292c4?q=80&w=1770&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')",
          }}
        >
          <div className="flex items-center justify-center w-full h-full bg-gray-900/40">
            <div className="text-center flex flex-col gap-4">
              <h1 className="text-3xl font-semibold text-white lg:text-4xl">
                Welcome to{" "}
                <span className="text-blue-400">{currentCity?.city}</span>
              </h1>
