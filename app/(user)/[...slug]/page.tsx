...
const CityDetailPage = async ({ params }: CityDetailPageProps
    ...
    const currentCity = await getCityById(cityId);
    const attorneys = await getAttorneysByCityId(cityId);
    const articles = await getArticlesByCityId(cityId);
    const faqs = await getAllFAQsByCityId(cityId);
    const reviews = await getReviewsByCityId(cityId);
    ...
    return (
        <section className="bg-white dark:bg-gray-900">
                            ...
                            <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        ...
                                        <p className="text-xs text-muted-foreground">{currentCity.stateName}</p>
                                    </CardContent>
                                </Card>
            ...
            <AttorneySection attorney={attorneys.response} city={params.slug[1]} />

            <BlogSection articles={articles.response} />
            <FAQs faqs={faqs.response} />
            <ReviewSection reviews={reviews.response} />
            {
                attorneys.response.length === 0 && articles.response.length === 0 && faqs.response.length === 0 && (
                    <div className="h-10 flex items-center justify-center text-gray-600">
                        No data available.
                    </div>
                )
            }
